...
<!-- CONTAINER HOUSING ALL CONTENTS IN THE DASHBOARD -->
	<div class="main_loader">
		...
		<div>
			...
			<p>Allow and Deny withdrawals on the system!!</p>
		</div>
		...
		<table width="100%" bgcolor="" class="main_table" cellpadding="10" cellspacing="0" align="center">
			<tr style="background:slateblue;color: wheat;">
				<th>S/N</th><th>Member ID</th><th>Full Name</th><th>Username</th><th>Email</th><th>Referal_ID</th><th>Balance</th><th>Ref balance</th><th>Registration date</th><th>Withdraw status</th><th>Access</th><th colspan="3" style="text-align:center;">Action</th>
			</tr>
			...
			<tr>
				...
				<td><form method="POST"><input type="hidden" name="row_id" value="<?php echo $user['id']; ?>"><input type="hidden" name="access" value="<?php echo $user['withdrawal_status']; ?>">
					<button type="submit" name="action_btn">
						<?php 
						if($user['withdrawal_status']=='allowed'){
							echo "Deny";
						} 
						else{
							echo "Allow";
...
if(isset($_POST['action_btn'])){
	...
	if($user_access=='allowed'){
		$up_access="update user_table set withdrawal_status='denied' where id='$user' ";
		$access_done=$conn->query($up_access);
		if($access_done){
			echo "<script>window.location='withdraw-setting'; </script>";
		}
	}
	else{
		$up_access="update user_table set withdrawal_status='allowed' where id='$user' ";
		$access_done=$conn->query($up_access);
		if($access_done){
			echo "<script>window.location='withdraw-setting'; </script>";
		}
