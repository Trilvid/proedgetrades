<div>
	...
	$user_sel="select * from withdrawal_table where status='processing' and type='withdraw' ";
...
<!-- CONTAINER HOUSING ALL CONTENTS IN THE DASHBOARD -->
	<div class="main_loader">
		...
		<div>
		<div>
			<h2>All system Withdrawals</h2>
			<p>Below are pending Withdrawals on the system!!</p>
		</div>
		...
		<table width="100%" bgcolor="" class="main_table" cellpadding="10" cellspacing="0" align="center">
			<tr style="background:slateblue;color: wheat;">
				<th>S/N</th><th>Username</th><th>Email</th><th>Amount</th><th>Withdrawal date</th><th>Coin</th><th>Wallet_address</th><th>Status</th><th colspan="3" style="text-align:center;">Action</th>
			</tr>
			...
			<tr>
				...
				<td><form method="POST" action="admin_action.php">
					...
					<input type="hidden" name="with_amt" value="<?php echo $user['amount']; ?>">
					<input type="hidden" name="withdraw_id" value="<?php echo $user['withdraw_id']; ?>">
					<button type="submit" name="confirm_withdraw">Confirm</button></form>
				</td>
				<td><form method="POST" action="admin_action.php">
					<input type="hidden" name="row_id" value="<?php echo $user['id']; ?>">
					<input type="hidden" name="with_amt" value="<?php echo $user['amount']; ?>">
					<input type="hidden" name="withdraw_id" value="<?php echo $user['withdraw_id']; ?>">
					<button type="submit" name="decline_withdraw">Decline</button></form>
				</td>
			</tr>
