<div>
	...
	$user_sel="select * from trading_table where status='active' and account_type='spot'";
...
<!-- CONTAINER HOUSING ALL CONTENTS IN THE DASHBOARD -->
	<div class="main_loader">
		...
		<div>
		<div>
			<h2>All system Active Real Trades</h2>
			<p>Below are Active trades on the system!!</p>
		</div>
		...
		<table width="100%" bgcolor="" class="main_table" cellpadding="10" cellspacing="0" align="center">
			...
			<tr>
				...
				<td><form method="POST" action="">
                    ...
                    <input type="number" name="trade_loss" value="" max='<?php echo $user['amount']; ?>' required min='1' id="trd_amt" placeholder='loss amount'>
					<button type="submit" name="loss_trade" style='background:red;'>close/loss</button></form>
				</td>
				<td><form method="POST" action="">
					<input type="hidden" name="row_id" value="<?php echo $user['id']; ?>">
					<input type="hidden" name="trade_amt" value="<?php echo $user['amount']; ?>">
					<input type="hidden" name="trader_id" value="<?php echo $user['trader_id']; ?>">
                    <input type="number" name="trade_loss" value="" max='' required min='1' id="trd_amt" placeholder='profit amount'>
					<button type="submit" name="profit_trade" style='background:green;'>close/profit</button></form>
				</td>
			</tr>
...
if(isset($_POST['loss_trade'])){
    ...
    $loss=$_POST['trade_loss'];


    // the returnig balance
    $return_tot=$trade_amt-$loss;

    // percentage loss
    $percent_loss=$loss/$trade_amt*100;
    ...
    $up_act="update user_table set spot=spot+'$return_tot' where id='$trader_id'";
    $up_act_done=$conn->query($up_act);

    if($up_act_done){
        $up_sql="update trading_table set profit='-$loss', total_return='$return_tot', percent_return='-$percent_loss', status='completed' where id='$row_id'";
        $up_sql_done=$conn->query($up_sql);

        if($up_sql_done){
            echo "<script>window.location='active-trades'; </script>";
        }
    }
...
// WORKING ON ADDING PROFIT

if(isset($_POST['profit_trade'])){
    ...
    $trade_amt=$_POST['trade_amt'];
    $trader_id=$_POST['trader_id'];
    $prof=$_POST['trade_loss'];


    // the returnig balance
    $return_tot=$trade_amt+$prof;

    // percentage loss
    $percent_prof=$prof/$trade_amt*100;
    ...
    if($up_act_done){
        $up_sql="update trading_table set profit='+$prof', total_return='$return_tot', percent_return='+$percent_prof', status='completed' where id='$row_id'";
        $up_sql_done=$conn->query($up_sql);
